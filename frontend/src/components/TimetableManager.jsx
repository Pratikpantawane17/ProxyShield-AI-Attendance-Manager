...
import { Plus, Edit2, Trash2, Calendar, Clock, BookOpen, CheckCircle, XCircle, Save, DoorOpen } from 'lucide-react';
...
const CLASSROOMS = Array.from({ length: 21 }, (_, i) => i + 1); // [1, 2, 3, ..., 21]
...
function TimetableManager() {
  ...
  const ASKED_KEY = 'sa_manager_notif_asked_v1';
  ...
  // On mount: detect notification support & permission state and show in-app banner (but don't auto request)
  useEffect(() => {
    ...
    if (Notification.permission === 'granted') {
      registerAndSendTokenToServer()
        ...
        .catch((err) => {
          ...
          toast.error('Failed to register for notifications. Check console for details.');
        });
      return;
    }
  ...
  // helper: ask browser permission then register FCM token
  const askBrowserNotificationPermission = async () => {
    if (!('Notification' in window) || !('serviceWorker' in navigator)) {
      setNotifSupported(false);
      toast.warn('Notifications are not supported in this browser.');
      return null;
    }
    ...
    try {
      ...
      if (permission === 'granted') {
        ...
        try {
          ...
          if (token) {
            toast.success('Notifications enabled - you will receive lecture reminders.');
          } else {
            toast.info('Notifications enabled (no token returned).');
          }
          return token;
        } catch (err) {
          console.error('registerAndSendTokenToServer error:', err);
          if (err && err.code && err.code.includes('messaging/permission-blocked')) {
            setPermissionBlocked(true);
            toast.error('Notification permission blocked in browser. Please enable notifications in browser settings.');
          } else {
        ...
        setPermissionBlocked(true);
        toast.error('Notification permission denied. You can enable it from browser settings later.');
        return null;
      } else {
        toast.info('Notification prompt dismissed. You can enable notifications later from the banner.');
        return null;
      }
    } catch (err) {
      console.error('askBrowserNotificationPermission error:', err);
  ...
  const addNewLectureForm = () => {
    setPendingLectures(prev => [...prev, { ...initialFormState, tempId: Date.now() }]);
  };
  ...
  const validateLecture = (lecture) => {
    ...
    if (lecture.daysOfWeek.length === 0) {
      return 'Please select at least one day';
    }
    if (!lecture.time) {
      return 'Please select lecture time';
    }
    if (!lecture.startDate || !lecture.endDate) {
      return 'Please select both start and end dates';
    }
    if (new Date(lecture.startDate) > new Date(lecture.endDate)) {
  ...
  const handleSubmitAll = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/teacher/timetable/bulk`, 
        { lectures: payload },
        { withCredentials: true }
      );

      const newLectures = payload.map((lecture, index) => ({
        ...lecture,
        id: response.data.ids?.[index] || Date.now() + index
      }));
      ...
      if (error.response) {
        if (error.response?.status === 401) {
          ...
          toast.error(error.response?.data?.message || 'Failed to add lectures!');
        }
      } else {
  ...
  const handleEdit = (lecture) => {
    setPendingLectures([{
      ...
      daysOfWeek: lecture.daysOfWeek,
      time: lecture.time,
      startDate: lecture.dateRange.startDate,
      endDate: lecture.dateRange.endDate,
      classroom: lecture.classroom,
      isActive: lecture.isActive,
      tempId: Date.now()
    }]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/teacher/timetable/${id}`, {
        withCredentials: true
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Manage your teaching schedule and notifications</p>
        </div>

        {notifSupported && showPermissionPrompt && (
          <div className="max-w-6xl mx-auto mb-4 p-4 rounded-lg bg-yellow-50 border border-yellow-200 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-gray-600">Allow browser notifications to receive reminders for your scheduled lectures. You will only be asked once.</div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  window.localStorage.setItem(ASKED_KEY, '1');
                  setShowPermissionPrompt(false);
                  toast.info('You can enable notifications later from the banner at the top.');
                }}
        ...
        {notifSupported && permissionBlocked && (
          <div className="max-w-6xl mx-auto mb-4 p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-800">
            Notification permission is blocked. To receive lecture reminders, enable notifications for this site in your browser settings (usually via the lock icon in the address bar).
          </div>
        )}

        {!notifSupported && (
          <div className="max-w-6xl mx-auto mb-4 p-3 rounded-lg bg-gray-50 border border-gray-200 text-sm text-gray-700">
            Notifications are not supported in this browser. Lecture reminders require a modern browser with service worker and notification support.
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 mb-8">
          ...
          <div className="space-y-8">
              ...
              <div key={lecture.tempId} className="relative border-2 border-gray-200 rounded-xl p-6 space-y-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {pendingLectures.length > 1 && (
                    <button
                      type="button"
                      onClick={() => removeLectureForm(lecture.tempId)}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange(lecture.tempId, e)}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={(e) => handleInputChange(lecture.tempId, e)}
                      className="w-full pl-12 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition appearance-none bg-white"
                    >
                      <option value="">Select classroom...</option>
                      {CLASSROOMS.map(room => (
                        <option key={room} value={room}>Classroom {room}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => toggleDaysDropdown(lecture.tempId)}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg text-left focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition bg-white"
                  >
                    ...
                    <div className="absolute z-10 w-full mt-2 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto">
                      {DAYS_OF_WEEK.map(day => (
                        <label key={day} className="flex items-center px-4 py-3 hover:bg-indigo-50 cursor-pointer transition">
                          <input
                            type="checkbox"
                            checked={lecture.daysOfWeek.includes(day)}
                            onChange={() => toggleDay(lecture.tempId, day)}
            ...
            <button
              ...
              className="w-full flex items-center justify-center px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 transition border-2 border-dashed border-gray-300"
            >
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t-2 border-gray-200">
              <button
                ...
                className="flex-1 flex items-center justify-center px-6 py-4 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-lg"
              >
              ...
              {editingId !== null && (
                <button
                  type="button"
                  onClick={cancelEdit}
                  className="flex-1 px-6 py-4 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 transition font-semibold"
                >
                  Cancel
                </button>
              )}
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Classroom</th>
                    ...
                    <tr key={lecture.id} className="hover:bg-gray-50 transition">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm font-medium text-gray-900">{lecture.subject}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <DoorOpen className="w-4 h-4 text-gray-400 mr-2" />
                          <span className="text-sm text-gray-900">Room {lecture.classroom}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div className="flex items-center">
                          ...
                          <span>{lecture.dateRange.startDate} to {lecture.dateRange.endDate}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {lecture.isActive ? (
